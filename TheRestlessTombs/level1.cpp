#include "level1.h"

Level1::Level1(b2World* world, ResourceManager* rm, Input* input, Camera* camera) : Scene::Scene(camera){
	this->rm = rm;
	this->input = input;
	this->world = world;
	currentRoom = 0;
	player = new Player(input, rm, camera, rm->GetShader("shader"), this->world);
	player->GiveTexture(rm->GetTexture("player"));
	player->CreateBody(this->camera->screenWidth/2, this->camera->screenHeight/2, 50, 75, true, false);
	wall = new Wall(true, this->camera, this->rm->GetShader("shader"), this->world);
	wall->GiveTexture(this->rm->GetTexture("wall"));
	wall->CreateBody(150, 0, 300, 74);
	wall2 = new Wall(false, this->camera, this->rm->GetShader("shader"), this->world);
	wall2->GiveTexture(this->rm->GetTexture("wall"));
	wall2->CreateBody(650, 0, 300, 74);
	wall3 = new Wall(false, this->camera, this->rm->GetShader("shader"), this->world);
	wall3->CreateBody(-37, this->camera->screenHeight/2, 74, 74 + this->camera->screenHeight);
	wall4 = new Wall(false, this->camera, this->rm->GetShader("shader"), this->world);
	wall4->CreateBody(this->camera->screenWidth/2, this->camera->screenHeight + 74, this->camera->screenWidth + 148, 74);
	wall5 = new Wall(false, this->camera, this->rm->GetShader("shader"), this->world);
	wall5->CreateBody(this->camera->screenWidth + 37, this->camera->screenHeight/2, 74, 74 + this->camera->screenHeight);
	wall6 = new Wall(false, this->camera, this->rm->GetShader("shader"), this->world);
	wall6->CreateBody(150, -170, 300, 260);
	wall7 = new Wall(false, this->camera, this->rm->GetShader("shader"), this->world);
	wall7->CreateBody(650, -170, 300, 260);
	wall8 = new Wall(true, this->camera, this->rm->GetShader("shader"), this->world);
	wall8->GiveTexture(this->rm->GetTexture("wall"));
	wall8->CreateBody(this->camera->screenWidth/2, -537, this->camera->screenWidth, 74);
	floor = new Renderable(this->rm->GetTexture("floorDetailed"), this->camera, this->rm->GetShader("shader"));
	floor->CreateBody(this->camera->screenWidth/2, this->camera->screenHeight/2 + 37, this->camera->screenWidth, this->camera->screenHeight, this->camera->screenWidth, this->camera->screenHeight);
	floor2 = new Renderable(this->rm->GetTexture("floorStandard"), this->camera, this->rm->GetShader("shader"));
	floor2->CreateBody(this->camera->screenWidth/2, -400, this->camera->screenWidth, 200, this->camera->screenWidth, 200);
	stair1 = new Renderable(this->rm->GetTexture("stairs"), this->camera, this->rm->GetShader("shader"));
	stair1->CreateBody(this->camera->screenWidth/2, -200 + 37, 200, 400, 200, 400);
	door = new Door(1, Direction::east, this->camera, this->rm->GetShader("shader"), this->world);
	door->CreateBody(this->camera->screenWidth - 25, -400, 200.0f);
	door->GiveTexture(this->rm->GetTexture("doorEast"));
	door2 = new Door(1, Direction::west, this->camera, this->rm->GetShader("shader"), this->world);
	door2->CreateBody(0 + 25, -400, 200.0f);
	door2->GiveTexture(this->rm->GetTexture("doorWest"));
	healthPotion1 = new HealthPotion(1, this->camera, this->rm->GetShader("shader"), this->world);
	healthPotion1->CreateBody(450, 150, 30, 36);
	healthPotion1->GiveTexture(this->rm->GetTexture("healthPotion"));
	damagePotion1 = new DamagePotion(1.0f, 5.0f, this->camera, this->rm->GetShader("shader"), this->world);
	damagePotion1->CreateBody(100, 300, 30, 36);
	damagePotion1->GiveTexture(this->rm->GetTexture("damagePotion"));
	speedPotion1 = new SpeedPotion(2.0f, -0.15f, 4.0f, this->camera, this->rm->GetShader("shader"), this->world);
	speedPotion1->CreateBody(150, 300, 30, 36);
	speedPotion1->GiveTexture(this->rm->GetTexture("speedPotion"));
	crate1 = new Crate(this->camera, this->rm->GetShader("shader"), this->world);
	crate1->CreateBody(450, 150, 50, 60, false, false);
	crate1->GiveTexture(this->rm->GetTexture("crate"));
	crate1->Store(healthPotion1);
	lootChest1 = new LootChest(100, this->player, this->rm->GetShader("text"), camera, this->rm->GetShader("shader"), this->world);
	lootChest1->CreateBody(400, 500, 75, 75, false, false);
	lootChest1->GiveTexture(this->rm->GetTexture("goldLootChestClosed"), this->rm->GetTexture("goldLootChestOpened"));
	babyOrc1 = new BabyOrc(this->player, 350.0f, this->rm, this->camera, this->rm->GetShader("shader"), this->world);
	babyOrc1->CreateBody(400, -300, 50, 50);
	babyOrc1->GiveTexture(this->rm->GetTexture("babyOrc"));
	/*psychoOrc1 = new PsychoOrc(this->player, 350.0f, this->rm, this->camera, this->rm->GetShader("shader"), this->world);
	psychoOrc1->CreateBody(500, 500, 50, 62);
	psychoOrc1->GiveTexture(this->rm->GetTexture("psychoOrc"));
	orc1 = new Orc(this->player, 350.0f, this->rm, this->camera, this->rm->GetShader("shader"), this->world);
	orc1->CreateBody(500, 500, 50, 75);
	orc1->GiveTexture(this->rm->GetTexture("orc"));
	armoredOrc1 = new ArmoredOrc(this->player, 350.0f, this->rm, this->camera, this->rm->GetShader("shader"), this->world);
	armoredOrc1->CreateBody(500, 500, 50, 75);
	armoredOrc1->GiveTexture(this->rm->GetTexture("armoredOrc"));
	necromancerOrc1 = new NecromancerOrc(1.5f, this->player, 250.0f, this->rm, this->camera, rm->GetShader("shader"), this->world);
	necromancerOrc1->CreateBody(555, 500, 50, 75);
	necromancerOrc1->GiveTexture(this->rm->GetTexture("necromancerOrc"));
	bomb1 = new Bomb(30.0f, 3.0f, 150.0f, 0.2f, this->rm->GetTexture("bomb"), this->rm->GetTexture("showCase"), this->camera, this->rm->GetShader("bomb"));
	bomb1->CreateBody(500, 500, 30, this->world);*/
	bomberOrc1 = new BomberOrc(this->player, 350.0f, this->rm, this->camera, this->rm->GetShader("shader"), this->world);
	bomberOrc1->CreateBody(500, 500, 50, 57);
	bomberOrc1->GiveTexture(this->rm->GetTexture("bomberOrc"));
	/*bossOrc = new BossOrc("BossOrc", this->player, 350.0f, this->rm, this->camera, this->rm->GetShader("shader"), this->world);
	bossOrc->CreateBody(500, 500, 75, 100);
	bossOrc->GiveTexture(this->rm->GetTexture("bossOrc"));*/
	gold1 = new Gold(30, this->camera, this->rm->GetShader("shader"), this->world);
	gold1->CreateBody(500, 400, 15, 15, true, true);
	gold1->GiveTexture(this->rm->GetTexture("gold"));

	currentRoom = 0;
	room1 = new Room(camera);
	this->AddChild(room1);
	room1->AddChild(floor);
	room1->AddChild(floor2);
	room1->AddChild(stair1);
	room1->AddChild(wall);
	room1->AddChild(wall2);
	room1->AddChild(wall3);
	room1->AddChild(wall4);
	room1->AddChild(wall5);
	room1->AddChild(wall6);
	room1->AddChild(wall7);
	room1->AddChild(wall8);
	room1->AddChild(door);
	room1->AddChild(door2);
	room1->AddChild(crate1);
	room1->AddChild(healthPotion1);
	room1->AddChild(damagePotion1);
	room1->AddChild(speedPotion1);
	room1->AddChild(lootChest1);
	room1->AddChild(babyOrc1);
	//room1->AddChild(psychoOrc1);
	//room1->AddChild(orc1);
	//room1->AddChild(armoredOrc1);
	//room1->AddChild(necromancerOrc1);
	//room1->AddChild(bomb1);
	room1->AddChild(bomberOrc1);
	room1->AddChild(gold1);
	//room1->AddChild(bossOrc);
	room1->SetActive(true);
	rooms.push_back(room1);

	room2 = new Room(camera);
	this->AddChild(room2);
	room2->SetActive(false);
	rooms.push_back(room2);

	this->AddChild(player);
}

Level1::~Level1() {
	delete player;
	delete wall;
	delete wall2;
	delete room1;
	delete floor;
	delete floor2;
	delete door;
	delete door2;
	delete room2;
	delete wall3;
	delete wall4;
	delete wall5;
	delete wall6;
	delete wall7;
	delete wall8;
	delete stair1;
	delete crate1;
	delete healthPotion1;
	delete damagePotion1;
	delete speedPotion1;
	delete lootChest1;
	delete babyOrc1;
	//delete psychoOrc1;
	//delete orc1;
	//delete armoredOrc1;
	//delete necromancerOrc1;
	//delete bomb1;
	delete bomberOrc1;
	//delete bossOrc;
	delete gold1;
}

void Level1::Update(float deltaTime) {
	// Check in which room the player is located, if the player is in a new room then load the new room
	int playerRoom = player->GetRoom();
	if (playerRoom != currentRoom) {
		rooms[currentRoom]->SetActive(false);
		this->RemoveChild(rooms[currentRoom]);
		currentRoom = playerRoom;
		rooms[currentRoom]->SetActive(true);
		this->AddChild(rooms[currentRoom]);
	}
	// RESET
	if (input->KeyPress(SDL_SCANCODE_R)) {
		crate1->Reset();
		damagePotion1->Reset();
		speedPotion1->Reset();
		babyOrc1->Reset();
		//psychoOrc1->Reset();
		//orc1->Reset();
		//armoredOrc1->Reset();
		//necromancerOrc1->Reset();
		//bomb1->Reset();
		bomberOrc1->Reset();
		//bossOrc->Reset();
		gold1->Reset();
	}
	this->UpdateChilderen(this, deltaTime);
}
