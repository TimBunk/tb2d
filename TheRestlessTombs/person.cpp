...
Person::Person(Camera* camera, Shader* shader) : Entity::Entity() {
	this->camera = camera;
	this->shader = shader;
    health = 0;
    damage = 0;
...
void Person::CreateBody(int x, int y, int w, int h, b2World* world) {
...
void Person::GiveTexture(Texture texture) {
	this->texture = texture;
}

glm::vec3 Person::GetPositionInPixels()
{
	glm::vec3 pos;
